...
try {
    ...
    $JBNSearch = winget.exe search -e --id Insomnia.Insomnia --accept-source-agreements
    $JBNOnlineVersion = (-split $JBNSearch[-1])[-2]
    
    # Get the locally installed version
    $JBNLocalSearch = winget.exe list -e --id Insomnia.Insomnia
    ...
    if ($JBNLocalVersion -ge $JBNOnlineVersion) {
        Write-Output "The device has the latest version installed."
        exit 0
    } else {
